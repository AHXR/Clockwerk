 ...
 */
public class MasterControlSound implements Runnable {
	
	/*
	 * These variables are related to the ID argument in "playDotaSound".
	 * I organized these to avoid having any sort of variable confusion 
	 * in the future.
	 * 
	 * I would put this in an enumeration but a method call is never a constant
	 * expression and switch - case only accepts constant expressions.
	 */
	public final int
		SOUND_START = 0,
		SOUND_END = 1,
	...
	static int
		i_volume_abs = 50; // This is the value of the GUI's volume control. This is changed via settings.
	...
	public static void changeVolume( float amount ) {
		...
		i_volume_abs = Math.round( amount );
	}
	...
	String returnAudioPath( int id ) {
		...
		switch( id ) {
			case SOUND_START: {
				s_return = GlobalSettingsSound.START.setting(); // "The gears are in motion"
				break;
			}
			case SOUND_END: {
				s_return = GlobalSettingsSound.END.setting(); // "Must be getting rusty here"
				break;
			}
			case SOUND_TRIGGER: {
				s_return = GlobalSettingsSound.TRIGGER.setting(); // "My gears turn"
				break;
			}
			case SOUND_WARNING: {
				s_return = GlobalSettingsSound.WARNING.setting(); // "Hurry the hands of the clock"
				break;
			}
			case SOUND_MUTE: {
				s_return = GlobalSettingsSound.MUTE.setting(); // "SILENCE"
				break;
			}
			case SOUND_UNMUTE: {
				s_return = GlobalSettingsSound.UNMUTE.setting(); // "Speak your last"
				break;
			}
		}
		return GlobalSettingsSound.DIR.setting() + s_return;
	}
	...
	public void playDotaSound( int sound_num ) {
		...
		if( !b_muted ) {
			try{
				 ...
				 */
				File
					s_file = new File( GlobalThemeControl.s_run_path + GlobalThemeControl.ThemeSettings.FOLDER.setting() + returnAudioPath( sound_num ) );
	        	...
	        	NotificationDialogClient.Error("Audio Error", "There was an error while trying to play the audio file.");
	            ex.printStackTrace();
	        }
		}
	}
